...
class CogDataset_v46(Dataset):
    def __init__(self, max_num_frames = 10, verbose=False, mode="test", image_size=256):
        ...
        self.train_rois = [f"roi{i}" for i in self.train_roi_indices[:120]]
        self.val_rois = [f"roi{i}" for i in self.train_roi_indices[120:]]
    ...
    def __getitem__(self, idx):
        ...
        self.num_frames = self.max_num_frames + 1
...
#         least_cloudy_image = msi[:,least_cld_day]
#         input_images = [msi[:, day_index] for day_index in cloudy_days[:3]]
...
#         print(cloudy_mask.shape)
#         print(day_counts.shape)
        
        return input_images, least_cloudy_image, cloudy_mask, day_counts
